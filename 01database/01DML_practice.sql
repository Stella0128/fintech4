...
# 테이블 칼럼명, 속성 조회 desc 테이블명;
desc film;

# 테이블에서 원하는 자료 조회하기 select 칼럼명 (*) from 테이블명;
select * from film;

# film 테이블에서 title과 release_year 칼럼만 보고 싶을 때 
select title, release_year from film;

# 칼럼 별명, 별칭 주기 (as)  as 생략 가능
select title as 영화제목, rental_rate 대여요금 from film;

# 특정 조건을 만족하는 행만 출력하기 where절
# seletct 칼럼명 from 테이블명 where 조건
# 문자 조건 where 칼럼명 = "조건" 
# rating이 PG 인 행만 조회
select * from film where rating="PG";

# 숫자 조건 where 칼럼명 = 숫자, > 숫자(초과), <숫자(미만), >=(이상), <=(이하)  기준이 왼쪽
# rental_rate 가 3 초과인 행 조회
...
select * from film;
# description 칼럼에서 drama가 포함된 행을 조회
# select * from film where description = "drama"; -> 정확하게 일치하는 항목만 조회 가능
...
# 정렬 (order by asc(오름차순), order by desc(내림차순)) 기본적으로 오름차순(asc 생략 가능)
# rental_rate칼럼을 기준으로 내림차순 정렬
...
# select *, count(*) as 영화수 from film group by rating; (X)  -> 묶는 기준이 존재하므로 전체를 조회할 경우 오류 발생
# having: group by를 한 결과에서 원하는 조건을 줄 때 where 대신 사용
...
# join: 서로 다른 테이블의 하나의 결과처럼 연결하는 방법 공통된 칼럼을 기준으로 연결
# 공통된 칼럼은 보통 primary key, foreign key 으로 연결되어 있음
...
select avg(rental_rate) as 평균대여요금 from film;  # 2.98
select title, rental_rate from film where rental_rate > 2.98;

select title, rental_rate from film where rental_rate > (select avg(rental_rate) as 평균대여요금 from film);

# 서브쿼리 실행 결과가 여러개일 때 in, any, all을 사용해야 함
# 영화 category가 'Comedy' 또는 'Action'인 장르의 제목 보기   -> 서브쿼리, join 두 방법으로 가능
select * from film;
select * from category;
select * from film_category;
# film의 category를 알기 위해서는 위의 세 테이블을 전부 봐야 함
# 왜 이렇게 저장해? 데이터의 중복을 막기 위해서
#원래는 따로따로 구해야 함
...
# 서브쿼리 이용해서 합치면
select title from film as f where f.film_id
in (select fc.film_id from film_category as fc
	where fc.category_id in (select c.category_id from category as c where c.name in ('Comedy', 'Action')));
 
# 카테고리 이름을 같이 조회하고 싶으면?
